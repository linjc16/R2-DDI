...
target_dir = '/home/v-xialiang/blob2_containter/v-xialiang/dmp/data/new_split/ind_raw/'
...
ind_train = "{}/{}".format(ssiddi_dir, 'pair_pos_neg_triples-fold1-train.csv')
ind_valid = "{}/{}".format(ssiddi_dir, 'pair_pos_neg_triples-fold1-s2.csv')
ind_test = "{}/{}".format(ssiddi_dir, 'pair_pos_neg_triples-fold1-s1.csv')
...
def build_raw():
    ...
    reb_dir = "{}/{}".format(target_dir, "new_build1")
    if not os.path.exists(reb_dir):
        os.makedirs(reb_dir)
    ...
    train_data = []
    train_csv = pd.read_csv(ind_train)
    for d1, d2, y, neg in zip(tqdm(train_csv['Drug1_ID']), train_csv['Drug2_ID'], train_csv['Y'], train_csv['Neg samples']):
    ...
    valid_data = []
    valid_csv = pd.read_csv(ind_test)
    for d1, d2, y, neg in zip(tqdm(valid_csv['Drug1_ID']), valid_csv['Drug2_ID'], valid_csv['Y'], valid_csv['Neg samples']):
