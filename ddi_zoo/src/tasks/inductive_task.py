...
@dataclass
class InductiveConfig(FairseqDataclass):
...
@register_task("inductive_task", dataclass=InductiveConfig)
class InductiveInteractionTask(FairseqTask):

    cfg: InductiveConfig

    def __init__(self, cfg: InductiveConfig, data_dictionary, label_dictionary):
        super().__init__(cfg)
    ...
    @classmethod
    def setup_task(cls, cfg: InductiveConfig, **kwargs):
    ...
    def load_dataset_src_tgt(self, split):
        ...
        drug_a_neg_prefix = self.get_path("", '{}.neg{}-neg{}.neg{}'.format(split, self.drug_a, self.drug_b, self.drug_a))
        drug_b_neg_prefix = self.get_path("", '{}.neg{}-neg{}.neg{}'.format(split, self.drug_a, self.drug_b, self.drug_b))
        if not MMapIndexedDataset.exists(drug_a_neg_prefix):
            raise FileNotFoundError("SMILES data {} not found.".format(drug_a_neg_prefix))
        if not MMapIndexedDataset.exists(drug_b_neg_prefix):
            raise FileNotFoundError("SMILES data {} not found.".format(drug_b_neg_prefix))

        if self.cfg.dataset_impl is None:
            dataset_impl = infer_dataset_impl(drug_a_neg_prefix)
        else:
            dataset_impl = self.cfg.dataset_impl

        drug_a_neg_dataset = make_dataset(drug_a_neg_prefix, impl=dataset_impl)
        assert drug_a_neg_dataset is not None

        drug_b_neg_dataset = make_dataset(drug_b_neg_prefix, impl=dataset_impl)
        assert drug_b_neg_dataset is not None

        drug_neg_a = AppendTokenDataset(
            TruncateDataset(
        ...
        if not MolMMapIndexedDataset.exists(drug_a_neg_prefix):
            raise FileNotFoundError("PyG data {} not found.".format(drug_a_neg_prefix))
        
        if not MolMMapIndexedDataset.exists(drug_b_neg_prefix):
            raise FileNotFoundError("PyG data {} not found.".format(drug_b_neg_prefix))

        drug_neg_a_graph = make_graph_dataset(drug_a_neg_prefix, impl=dataset_impl)
        assert drug_neg_a_graph is not None
        drug_neg_a_graph = Tensor2Data(drug_neg_a_graph)

        drug_neg_b_graph = make_graph_dataset(drug_b_neg_prefix, impl=dataset_impl)
        assert drug_neg_b_graph is not None
        drug_neg_b_graph = Tensor2Data(drug_neg_b_graph)
        ...
        dataset = {
            ...
            "drug_a_seq_neg": {
                "src_tokens": LeftPadDataset(drug_neg_a, pad_idx=self.source_dictionary.pad()),
                "src_lengths": NumelDataset(drug_neg_a),
            },
            "drug_b_seq_neg": {
                "src_tokens": LeftPadDataset(drug_neg_b, pad_idx=self.target_dictionary.pad()),
                "src_lengths": NumelDataset(drug_neg_b),
            },
            "drug_a_graph_neg": {"graph": drug_neg_a_graph,},
            "drug_b_graph_neg": {"graph": drug_neg_b_graph,},
        }
    ...
    def tsne_inference_step(self, sample, model, criterion):
        model.eval()
        with torch.no_grad():
            a_embed, b_embed = criterion.tsne_inference(model, sample)
