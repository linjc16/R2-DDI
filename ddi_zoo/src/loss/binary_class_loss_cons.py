...
@dataclass
class BinaryClassConsNegSigmoidConfig(FairseqDataclass):
...
@register_criterion("binary_class_loss_cons", dataclass=BinaryClassConsNegSigmoidConfig)
class BinaryClassNegConsSigmoidCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if isinstance(pos_logits, tuple):
            ...
            reg_loss = F.mse_loss(F.logsigmoid(pos_logits), F.logsigmoid(pos_logits_deltaH)) # F.logsigmoid(pos_logits), F.logsigmoid(pos_logits_deltaH)
            ...
            # reg_loss_neg = F.mse_loss(neg_logits, neg_logits_ori)
            # reg_loss_neg += F.mse_loss(neg_logits_ori, neg_logits_deltaT)
            # reg_loss_neg += F.mse_loss(neg_logits_ori, neg_logits_deltaH)

            # reg_loss_neg = reg_loss_neg / 3.

            # reg_loss += reg_loss_neg
            reg_loss += F.mse_loss(F.logsigmoid(-neg_logits), F.logsigmoid(-neg_logits_ori))
