...
class BinaryClassCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if isinstance(pos_logits, tuple):
            ...
            # pos_weights = 1 # torch.pow(1 - F.sigmoid(pos_logits) , 2)
            # neg_weights = torch.pow(F.sigmoid(neg_logits) , 2)
            # loss = (- pos_weights * F.logsigmoid(pos_logits)).mean() + \
            #        (- neg_weights * torch.log(1 - F.sigmoid(neg_logits))).mean()
